...
pub async fn create_new_transaction(
    ...
    ledger.transformer.async_append(&ledger, vec![trx]).await?;
...
pub async fn upload_transaction_document(
    ...
    while let Some(field) = multipart.next_field().await.unwrap() {
        ...
        ledger.transformer.async_save(&ledger, striped_path_string, &content_buf).await?;
    ...
    let mut content = String::from_utf8(ledger.transformer.async_get(source_file_path.clone()).await?).unwrap();
    content.insert(span_info.span_end, '\n');
    content.insert_str(span_info.span_end + 1, &metas_content);
    ledger.transformer.async_save(&ledger, source_file_path, content.as_bytes()).await?;
    reload_sender.reload();
    ResponseWrapper::json("Ok".to_string())
}
