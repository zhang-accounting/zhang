...
export default function ToolList() {
  ...
  const items = toolItems.map((item) => (
    <Link to={item.uri} key={item.title} className="col-span-3 bg-gray-100 dark:bg-dark-700 w-full  h-full p-4 rounded-md transition-all duration-150 ease-in-out relative hover:shadow-md  after:content-[''] after:block after:pb-[100%]" >
