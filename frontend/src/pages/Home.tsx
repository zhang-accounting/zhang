...
import { STATISTIC } from '../gql/statistic';
...
const build_chart_data = (data: StatisticResponse) => {
  const dates = sortBy(Object.keys(data.detail).map(date => [date, new Date(date)]), item => item[1]);
  ...
  const isLogarithmic = false;
  const min = 0;
  // const min = Math.min.apply(null, total_dataset) - 50;
  // if (isLogarithmic) {
  //   total_dataset = total_dataset.map(item => item - min);

  const income_dataset = sequencedDate.map(date => -1 * parseFloat(data.detail[date]?.[AccountType.Income]?.number?? 0))
  const expense_dataset =  sequencedDate.map(date => parseFloat(data.detail[date]?.[AccountType.Expenses]?.number ?? 0))
