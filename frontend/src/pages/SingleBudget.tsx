...
function SingleBudget() {
    ...
    const {
        data: budget_interval_event,
        error: event_error
    } = useSWR<BudgetIntervalEventResponse[]>(`/api/budgets/${budgetName}/interval/${date.getFullYear()}/${date.getMonth() + 1}`, fetcher);
        ...
        <Container fluid>
            <Group position="apart" py="md" px="sm" align="center">
                <Stack>
                    ...
                    <Group>
                        {budget_info.related_accounts.map(account => <Badge key={account} color="pink"
                                                                            variant="filled">{account}</Badge>)}
                    </Group>
                </Stack>
                <Group spacing={"lg"}>
                    <Stack spacing="xs" align={"end"}>
                        <Text size="xs" color={"dimmed"}>Assigned Amount</Text>
                        <Amount amount={budget_info.assigned_amount.number}
                                currency={budget_info.assigned_amount.currency}></Amount>
                    </Stack>
                    <Stack spacing="xs" align={"end"}>
                        <Text size="xs" color={"dimmed"}>Activity Amount</Text>
                        <Amount amount={budget_info.activity_amount.number}
                                currency={budget_info.activity_amount.currency}></Amount>
                    </Stack>
                    <Stack spacing="xs" align={"end"}>
                        <Text size="xs" color={"dimmed"}>Available Amount</Text>
            ...
            <Table verticalSpacing="xs" withBorder>
                ...
                {(budget_interval_event ?? []).map(it => {
                        return (<tr>
                            ...
                            <td>{!("event_type" in it) && <Badge color="pink"
                                                                 variant="filled">{it.account}</Badge>}</td>
                            <td style={{textAlign: 'end'}}>{"event_type" in it &&
                                <Amount amount={it.amount.number} currency={it.amount.currency}/>}</td>
                            <td style={{textAlign: 'end'}}>{!("event_type" in it) &&
                                <Amount amount={it.inferred_unit_number} currency={it.inferred_unit_commodity}/>}</td>
                        </tr>)
