...
export default function AccountDocumentLine(props: Props) {
  ...
  return (
    <div className="flex flex-col border rounded-sm shadow-sm" onClick={isDocumentAnImage(props.path) ? () => props.onClick(props.path) : undefined}>
      <div 
        className="relative after:content-[''] after:block after:pb-[75%]"
      >
        {canPreview ? (
          <img
          ...
          <p className="absolute inset-0 w-full h-full flex items-center justify-center text-center bg-gray-100 text-gray-500">This document cannot be previewed</p>
        )}
      </div>
